...
typedef void(__stdcall* AudioCallback)(const BYTE* data, DWORD length);
AudioCallback g_audioCallback = nullptr;

//Events to calling from C# app or else.
extern "C" __declspec(dllexport) void __stdcall SetAudioCallback(AudioCallback callback) {
    g_audioCallback = callback;
    std::cout << "SetAudioCallback called." << std::endl;
}

extern "C" __declspec(dllexport) HRESULT StartCaptureAsync(DWORD processId, bool includeProcessTree,
    WORD kanal, DWORD sampleOrani, WORD derinlik) {
    
    channel = kanal;
    sampleRate = sampleOrani;
    bitsperSample = derinlik;
    ...
    if (FAILED(hr))
    {
        ...
        std::cout << "Application loopback is started." << std::endl;
        Sleep(4294967295); // I have added this sleep code to working project. Idk why we are adding this.. :/
    }
    return hr;
}

extern "C" __declspec(dllexport) HRESULT StopCaptureAsync(CLoopbackCapture* instance) {
    loopbackCapture.StopCaptureAsync();
    std::cout << "Application loopback is stoped." << std::endl;
    return S_OK;
}
    ...
    m_activateResult = SetDeviceStateErrorIfFailed([&]()->HRESULT
        {
            ...
            m_CaptureFormat.nChannels = channel;
...
HRESULT CLoopbackCapture::OnAudioSampleRequested()
{
    ...
    while (SUCCEEDED(m_AudioCaptureClient->GetNextPacketSize(&FramesAvailable)) && FramesAvailable > 0)
    {
        ...
        // Write File
        if (m_DeviceState != DeviceState::Stopping)
        {// C# tarafýna veriyi gönder
            //std::cout << "Captured bytes from C sharp: " << cbBytesToCapture << std::endl;
            if (g_audioCallback)
            {
                //std::cout << "Captured bytes from C sharp: " << cbBytesToCapture << std::endl;
                g_audioCallback(Data, cbBytesToCapture);
